...
namespace CTRPluginFramework
{
  ...
  PluginMenuHome::PluginMenuHome(std::string &name, bool showNoteBottom) : _noteTB("", "", showNoteBottom ? IntRect(20, 46, 280, 124) : IntRect(40, 30, 320, 180)), _showStarredBtn(Button::Toggle | Button::Sysfont | Button::Rounded, "Favorite", IntRect(30, 70, 120, 30), Icon::DrawFavorite), _hidMapperBtn(Button::Sysfont | Button::Rounded, "Mapper", IntRect(165, 70, 120, 30), Icon::DrawController), _gameGuideBtn(Button::Sysfont | Button::Rounded, "Game Guide", IntRect(30, 105, 120, 30), Icon::DrawGuide), _searchBtn(Button::Sysfont | Button::Rounded, "Search", IntRect(165, 105, 120, 30), Icon::DrawSearch), _arBtn(Button::Sysfont | Button::Rounded, "ActionReplay", IntRect(30, 140, 120, 30)), _toolsBtn(Button::Sysfont | Button::Rounded, "Tools", showNoteBottom ? IntRect(99, 172, 120, 30) : IntRect(165, 140, 120, 30), Icon::DrawTools), /**_closeBtn(*this, nullptr, IntRect(275, 24, 20, 20), Icon::DrawClose)**/ _keyboardBtn(Button::Icon, IntRect(130, 30, 25, 25), Icon::DrawKeyboard), _controllerBtn(Button::Icon, IntRect(170, 30, 25, 25), Icon::DrawGameController25), _AddFavoriteBtn(Button::Icon | Button::Toggle, IntRect(50, 30, 25, 25), Icon::DrawAddFavorite), _InfoBtn(Button::Icon | Button::Toggle, IntRect(90, 30, 25, 25), Icon::DrawInfo), _submenu{{"enable all", "disable all"}}
  {
...
#define IsUnselectableEntry(item) \
  (item->IsEntry() && item->AsMenuEntryImpl()._flags.isUnselectable)
  static u32 SelectableEntryCount(MenuFolderImpl &folder)
  {
    ...
    for (u32 i = 0; i < folder.ItemsCount(); i++)
    {
      ...
      if (item->IsEntry() &&
          item->AsMenuEntryImpl()._flags.isUnselectable)
        continue;
      else
        count++;
    }
  ...
  void PluginMenuHome::_ProcessEvent(Event &event)
  {
    ...
    case Event::KeyDown:
    {
      if (fastScroll.HasTimePassed(Milliseconds(800)) &&
          inputClock.HasTimePassed(Milliseconds(100)))
      {
    ...
    case Event::KeyPressed:
    {
      ...
      case Key::B:
      {
        ...
        if (folder->_owner != nullptr &&
            folder->_owner->OnAction != nullptr)
    ...
    *********************************/
    if (folder->ItemsCount() > 0 && event.key.code != Key::Touchpad &&
    ...
    **************************/
    if (folder->ItemsCount() > 0 &&
        _selector < static_cast<int>(folder->ItemsCount()))
    {
      item = folder->_items[_selector];
  ...
  void PluginMenuHome::_Update(Time delta)
  {
    ...
    *************/
    ...
    if (_selectedTextSize >= 280 &&
        _scrollClock.HasTimePassed(Seconds(2)))
    {
    ...
    if (folder && (*folder)[_selector])
    {
      ...
      else if ((*folder)[_selector]->IsEntry())
      {
        ...
        if (!ShowNoteBottom)
        {
          ...
          _controllerBtn.Enable(e->_owner != nullptr &&
                                e->_owner->Hotkeys.Count() > 0);
        }
        if (e->HasNoteChanged())
        {
